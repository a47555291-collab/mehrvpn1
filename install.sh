...
if [[ -z "$DOMAIN" ]]; then read -r -p 'Panel domain (leave empty for IP/HTTP): ' DOMAIN; fi
if [[ -n "$DOMAIN" && -z "$EMAIL" ]]; then read -r -p 'LetsEncrypt email: ' EMAIL; fi
VPN_REMOTE="${MEHRVPN1_VPN_REMOTE:-$DOMAIN}"
[[ -n "$VPN_REMOTE" ]] || VPN_REMOTE="$(curl -4fsS --max-time 10 https://api.ipify.org || true)"
[[ -n "$VPN_REMOTE" ]] || die 'Could not determine public IPv4. Set MEHRVPN1_VPN_REMOTE=... and retry.'
...
rm -rf "$ROOT"; install -d "$ROOT"; cp -a "$SRC_TREE/." "$ROOT/"; chown -R root:root "$ROOT"
...
if [[ -n "$DOMAIN" ]]; then SESSION_SECURE=true; else SESSION_SECURE=false; fi
cat > /etc/mehrvpn1.env <<EOF
MEHRVPN1_SESSION_SECURE=$SESSION_SECURE
MEHRVPN1_VPN_REMOTE=$VPN_REMOTE
EOF
chmod 0644 /etc/mehrvpn1.env
...
if [[ -n "$DOMAIN" ]]; then PANEL_URL="https://$DOMAIN/"; else PANEL_URL="http://$VPN_REMOTE/"; fi
...
============================================================
 ...
 Credentials: /root/.mehrvpn1-admin
============================================================
EOF
